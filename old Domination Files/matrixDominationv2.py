...
##allDominatingSets = findAll(final, m, weights, verbose=False)
##print("Dominating Set:", allDominatingSets)
##stream = open(f"m{m}n{n}Results.txt", "w")
##stream.write(f"m = {m}, n = {n}, k = {weights}")
##stream.write(f"Dominating Set: {allDominatingSets}")
##stream.close()
...
k = 2
start1 = tm.time()
weights = binary_lists_with_weight(n, k)
count1 = 0
for i in weights:
    count1 += 1
    print(i)
end1 = tm.time()
print(end1 - start1, count1)
start2 = tm.time()
print()

weightsRecur = binary_lists_with_weightInRecurv(n, k)
count2 = 0
for i in weightsRecur:
    count2 += 1
    print(i)
end2 = tm.time()
print(end2 - start2, count2)
